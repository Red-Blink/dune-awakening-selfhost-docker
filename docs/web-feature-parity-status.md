...
| Broadcast | Partial | Publishes RedBlink `ServiceBroadcast` envelopes to `dune-rmq-game` `heartbeats/notifications`, but live validation showed `publish=ok ... label=web-broadcast` with no visible in-game message and no clear survival/director consumption log. Attempts are visible in admin history as `web-broadcast` or `web-shutdown-broadcast`. | `admin-server/src/rmq.js`, `admin-server/src/server.js`, `web/src/api/admin.ts`, `web/src/App.tsx` | Verify the correct exchange/routing/envelope for in-game display or add a RedBlink CLI path that confirms delivery. | Yes as a publish test with explicit unverified in-game visibility. |
